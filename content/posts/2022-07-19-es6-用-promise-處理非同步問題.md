...
draft: false
---
ES6 新增了一個新語法：Promise。當非同步發生時，如果我們想要確保執行的順序是在完成非同步事件後，才往下執行的話就可以用 Promise 處理。(過去會用 Callback， ES7 則可以用語法糖 Async Await)
...
let arrCookFoodPromise = [cookFoodPromise('花雕雞泡麵', 5, 1500), cookFoodPromise('麻油雞泡麵', 5, 3500)]
