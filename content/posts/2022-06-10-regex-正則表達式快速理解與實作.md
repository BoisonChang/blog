...
如果這些規則一大串又簡寫時就算是高手也很難瞬間看懂，這邊推薦一個可以在剛開始透過圖像理解的網站 [CyrilEx Regex Tester](https://extendsclass.com/regex-tester.html)，一開始要打比較複雜的規則時建議貼上去看圖。
...
const regex = /(?<=\s)\d+(?=元)/
'數量 2,實付金額 990元'.match(regex) // ["990", index: 10, ...]
```
